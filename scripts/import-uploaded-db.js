...
const SQLITE_BACKUP_PATH = path.join(__dirname, '..', 'data', 'backup-import.db');
    ...
    return new Promise((resolve, reject) => {
        const db = new sqlite3.Database(SQLITE_BACKUP_PATH, sqlite3.OPEN_READONLY, async (err) => {
            ...
            try {
                ...
                await importTable(db, 'pdf_tickets', [
                    'id', 'loja', 'data', 'filename', 'filepath', 'uploaded_by', 'uploaded_at'
                ]);
                
                await importTable(db, 'pdf_rankings', [
                    'id', 'loja', 'data', 'filename', 'filepath', 'pa', 'preco_medio',
                    'atendimento_medio', 'uploaded_by', 'uploaded_at'
                ]);
