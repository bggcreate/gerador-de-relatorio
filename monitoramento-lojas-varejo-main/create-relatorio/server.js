...
// Usando armazenamento em memória, o arquivo fica em req.file.buffer
...
let db = new sqlite3.Database(DB_PATH, err => {
    if (err) return console.error("Erro fatal ao conectar ao DB:", err.message);
    ...
    db.serialize(() => {
        ...
        db.run(`CREATE TABLE IF NOT EXISTS relatorios (
            ...
            gerente_entrada TEXT, gerente_saida TEXT,
            clientes_monitoramento INTEGER, vendas_monitoramento INTEGER,
            clientes_loja INTEGER, vendas_loja INTEGER,
            total_vendas_dinheiro REAL,
            ticket_medio TEXT, pa TEXT,
            quantidade_trocas INTEGER,
            nome_funcao_especial TEXT, quantidade_funcao_especial INTEGER,
...
app.post('/api/process-pdf', requirePageLogin, upload.single('pdfFile'), async (req, res) => {
    ...
    const safeParseFloat = (str) => {
        if (!str) return 0;
        // Limpa a string de R$, espaços, etc.
        const cleaned = String(str).replace(/[R$\s]/g, '');

        // Se a string contém vírgula, assume o formato brasileiro (1.234,56)
        // Remove os pontos de milhar e troca a vírgula por ponto decimal.
        if (cleaned.includes(',')) {
            return parseFloat(cleaned.replace(/\./g, '').replace(',', '.'));
        }

        // Se não houver vírgula, pode ser um formato como "12345" ou "123.45".
        // O parseFloat padrão do JS já lida com isso corretamente.
    ...
    try {
        ...
        // Regex mais robusto para capturar números com ou sem decimais
        const valoresTotais = linhaLimpa.match(/(\d{1,3}(?:\.\d{3})*,\d{2})|(\d+\.\d{2})|(\d+)/g);

        if (!valoresTotais || valoresTotais.length < 7) {
            console.error("Valores extraídos:", valoresTotais); // Ajuda a debugar
            throw new Error("Não foi possível extrair os valores corretamente da linha Totais.");
        }

        // Usa a nova função segura em todos os campos numéricos
        const totalVendasValor = safeParseFloat(valoresTotais[0]);
        const pa = safeParseFloat(valoresTotais[valoresTotais.length - 4]);
        const ticketMedio = safeParseFloat(valoresTotais[valoresTotais.length - 3]);
        ...
        res.json({
            ...
            data: {
                ...
                // Formata os valores corretamente para a UI
...
app.post('/api/usuarios', requirePageLogin, requireAdmin, (req, res) => {
    ...
    db.run('INSERT INTO usuarios (username, password, role) VALUES (?, ?, ?)', [username, password, role], function (err) {
...
app.put('/api/usuarios/:id', requirePageLogin, requireAdmin, (req, res) => {
    ...
    const sql = password ? 'UPDATE usuarios SET username = ?, password = ?, role = ? WHERE id = ?' : 'UPDATE usuarios SET username = ?, role = ? WHERE id = ?';
...
app.post('/api/lojas', requirePageLogin, (req, res) => {
    ...
    db.run('INSERT INTO lojas (nome, status, funcao_especial, observacoes) VALUES (?, ?, ?, ?)', [nome, status, funcao_especial, observacoes], function (err) {
...
app.put('/api/lojas/:id', requirePageLogin, (req, res) => {
    const { id } = req.params;
    const { nome, status, funcao_especial, observacoes } = req.body;
    db.run('UPDATE lojas SET nome = ?, status = ?, funcao_especial = ?, observacoes = ? WHERE id = ?', [nome, status, funcao_especial, observacoes, id], function (err) {
...
const processarRelatorio = (r) => {
    ...
    try {
        ...
        vendedores_processados = vendedores.map(v => ({ ...v, tx_conversao: (v.atendimentos > 0 ? ((v.vendas / v.atendimentos) * 100) : 0).toFixed(2) }));
    } catch (e) { }
...
app.post('/api/relatorios', requirePageLogin, (req, res) => {
    const d = req.body;
    const sql = `INSERT INTO relatorios (loja, data, hora_abertura, hora_fechamento, gerente_entrada, gerente_saida, clientes_monitoramento, vendas_monitoramento, clientes_loja, vendas_loja, total_vendas_dinheiro, ticket_medio, pa, quantidade_trocas, quantidade_omni, quantidade_funcao_especial, vendedores, enviado_por_usuario, vendas_cartao, vendas_pix, vendas_dinheiro) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`;
    const params = [
        d.loja, d.data, d.hora_abertura, d.hora_fechamento, d.gerente_entrada, d.gerente_saida,
        parseInt(d.clientes_monitoramento, 10) || 0,
        parseInt(d.vendas_monitoramento, 10) || 0,
        parseInt(d.clientes_loja, 10) || 0,
        parseInt(d.vendas_loja, 10) || 0,
        parseFloat(String(d.total_vendas_dinheiro).replace(/[R$\s.]/g, '').replace(',', '.')) || 0,
        d.ticket_medio || 'R$ 0,00',
        d.pa || '0.00',
        parseInt(d.quantidade_trocas, 10) || 0,
        parseInt(d.quantidade_omni, 10) || 0,
        parseInt(d.quantidade_funcao_especial, 10) || 0,
        d.vendedores || '[]',
        req.session.username,
        parseInt(d.vendas_cartao, 10) || 0,
        parseInt(d.vendas_pix, 10) || 0,
        parseInt(d.vendas_dinheiro, 10) || 0
    ];
    db.run(sql, params, function (err) {
        if (err) { console.error("Erro ao inserir relatório:", err.message); return res.status(500).json({ error: 'Falha ao salvar relatório.' }); }
        res.status(201).json({ success: true, id: this.lastID });
    });
});
...
app.put('/api/relatorios/:id', requirePageLogin, (req, res) => {
    const { id } = req.params;
    const d = req.body;
    const sql = `UPDATE relatorios SET loja=?, data=?, hora_abertura=?, hora_fechamento=?, gerente_entrada=?, gerente_saida=?, clientes_monitoramento=?, vendas_monitoramento=?, clientes_loja=?, vendas_loja=?, total_vendas_dinheiro=?, ticket_medio=?, pa=?, quantidade_trocas=?, quantidade_omni=?, quantidade_funcao_especial=?, vendedores=?, vendas_cartao=?, vendas_pix=?, vendas_dinheiro=? WHERE id=?`;
    const params = [
        d.loja, d.data, d.hora_abertura, d.hora_fechamento, d.gerente_entrada, d.gerente_saida,
        parseInt(d.clientes_monitoramento, 10) || 0,
        parseInt(d.vendas_monitoramento, 10) || 0,
    ...
    db.run(sql, params, function (err) {
        if (err) {
            ...
            return res.status(500).json({ error: 'Falha ao atualizar o relatório.' });
        }
...
const formatarRelatorioTexto = (r) => {
    ...
    if (rp.vendedores_processados && rp.vendedores_processados.length > 0) {
        equipeInfo = rp.vendedores_processados.map(v => {
            return `${v.nome}: ${v.atendimentos} Atendimentos / ${v.vendas} Vendas / ${v.tx_conversao}%`;
        }).join('\n');
    }
...
app.get('/api/relatorios/:id/txt', requirePageLogin, (req, res) => {
    const sql = `
        SELECT r.*, l.funcao_especial
        FROM relatorios r
        LEFT JOIN lojas l ON r.loja = l.nome
        WHERE r.id = ?
    `;
...
app.get('/api/ranking', requirePageLogin, (req, res) => {
    let whereClauses = []; let params = [];
    if (req.query.data_inicio) { whereClauses.push('r.data >= ?'); params.push(req.query.data_inicio); }
    if (req.query.data_fim) { whereClauses.push('r.data <= ?'); params.push(req.query.data_fim); }
...
app.get('/api/dashboard/chart-data', requirePageLogin, (req, res) => {
    ...
    let whereClauses = []; let params = [];
    if (loja && loja !== 'todas') { whereClauses.push('loja = ?'); params.push(loja); }
    if (data_inicio) { whereClauses.push('data >= ?'); params.push(data_inicio); }
    if (data_fim) { whereClauses.push('data <= ?'); params.push(data_fim); }
    if (whereClauses.length === 0) {
        const date = new Date(); date.setDate(date.getDate() - 30);
        const startDate = date.toISOString().slice(0, 10);
        whereClauses.push('data >= ?'); params.push(startDate);
    }
...
app.post('/api/demandas', requirePageLogin, (req, res) => {
    ...
    db.run('INSERT INTO demandas (loja_nome, descricao, tag, criado_por_usuario) VALUES (?, ?, ?, ?)', [loja_nome, descricao, tag, req.session.username], function (err) {
...
app.put('/api/demandas/:id/concluir', requirePageLogin, (req, res) => {
    db.run("UPDATE demandas SET status = 'concluido', concluido_por_usuario = ?, concluido_em = CURRENT_TIMESTAMP WHERE id = ?", [req.session.username, req.params.id], function (err) {
...
app.delete('/api/demandas/:id', requirePageLogin, (req, res) => {
    db.run("DELETE FROM demandas WHERE id = ?", [req.params.id], function (err) {
...
app.get('/api/backup/info', requirePageLogin, requireAdmin, (req, res) => {
    try {
        ...
        res.json({ sizeMB: fileSizeInMegabytes.toFixed(2) });
    } catch (error) {
        res.status(500).json({ error: 'Não foi possível obter informações do banco de dados.' });
    }
});
app.get('/api/backup/download', requirePageLogin, requireAdmin, (req, res) => {
    const date = new Date().toISOString().slice(0, 10);
    const fileName = `backup_reports_${date}.db`;
...
app.delete('/api/backup/clear', requirePageLogin, requireAdmin, (req, res) => {
    db.serialize(() => {
        db.run("DELETE FROM relatorios", (err) => { if (err) return res.status(500).json({ error: "Erro ao limpar relatórios." }); });
        db.run("DELETE FROM demandas", (err) => { if (err) return res.status(500).json({ error: "Erro ao limpar demandas." }); });
        db.run("VACUUM", (err) => {
             if (err) return res.status(500).json({ error: "Erro ao compactar o banco de dados." });
             res.json({ success: true, message: 'Relatórios e demandas foram limpos com sucesso.' });
        });
    });
});
...
// =================================================================
app.post('/api/backup/restore', requirePageLogin, requireAdmin, upload.single('backupFile'), (req, res) => {
    if (!req.file) {
        return res.status(400).json({ error: "Nenhum arquivo de backup foi enviado." });
    }

    // O conteúdo do arquivo está no buffer da memória 
    const backupBuffer = req.file.buffer;

    // 1. Fechar a conexão com o banco de dados atual para liberar o arquivo.
    db.close((err) => {
        if (err) {
            console.error("Erro ao fechar o DB antes de restaurar:", err.message);
            return res.status(500).json({ error: "Não foi possível fechar a conexão com o banco de dados atual." });
        }

        // 2. Usar fs.writeFile para escrever o buffer do backup no arquivo de banco de dados.
        fs.writeFile(DB_PATH, backupBuffer, (err) => {
            if (err) {
                console.error("Falha ao escrever o arquivo de backup:", err.message);
                // Tenta reconectar ao banco de dados original para não deixar o servidor inoperante.
                db = new sqlite3.Database(DB_PATH);
                return res.status(500).json({ error: "Falha ao substituir o arquivo de banco de dados." });
            }

            // 3. Se a escrita for bem-sucedida, reconecte-se ao novo banco de dados (o backup).
            db = new sqlite3.Database(DB_PATH, (err) => {
                if (err) {
                    console.error("DB restaurado, mas falha ao reconectar:", err.message);
                ...
                res.json({ success: true, message: "Banco de dados restaurado com sucesso. A página será recarregada." });
            });
        });
    });
});
