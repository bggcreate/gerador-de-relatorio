...
void * RoutineLoginThread(LPVOID lpPara)
{
    ...
    while (it != g_DeviceList.end())
    {
        ...
        if (pDeviceInfo->nBelongThread == nThreadID)
        {
            ...
            pDeviceInfo->lLoginHandle = CLIENT_LoginEx2(pDeviceInfo->szDevIp, pDeviceInfo->nPort,pDeviceInfo->szUserName, pDeviceInfo->szPassWord,EM_LOGIN_SPEC_CAP_TCP, NULL, &deviceInfo, &error);
			
            ConvertLoginError2String(error , pDeviceInfo->strErrorCode);
