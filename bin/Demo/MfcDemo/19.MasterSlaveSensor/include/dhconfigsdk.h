...
#define MAX_CROWD_DISTRI_MAP_REGION_POINT_NUM	4				// 区域检测坐标个数
...
//EVENT_IVS_MAN_NUM_DETECTION(立体视觉区域内人数统计事件)
...
// 事件类型EVENT_IVS_STEREO_FIGHTDETECTION(ATM舱内打架事件)对应的规则配置
typedef struct tagCFG_IVS_STEREO_FIGHTDETECTION_INFO
{
	...
	int					nDetectRegionPoint;										// 检测区域顶点数
	CFG_POLYGON			stuDetectRegion[MAX_POLYGON_NUM];						// 检测区域
	int					nSensitivity;											// 灵敏度, 1-10, 默认值6
	int					nMaxHeight;												// 最大检测高度,单位cm
	int 				nMinHeight;												// 最小检测高度,单位cm
	BYTE                bReserved[512]; 										// 保留字节
} CFG_IVS_STEREO_FIGHTDETECTION_INFO;

// 事件类型EVENT_IVS_STEREO_TAILDETECTION(ATM舱内尾随事件)对应的规则配置
typedef struct tagCFG_IVS_STEREO_TAILDETECTION_INFO
{
	// 信息
	char				szRuleName[MAX_NAME_LEN];								// 规则名称,不同规则不能重名
	BOOL				bRuleEnable;											// 规则使能
	...
	int					nDetectRegionPoint;										// 检测区域顶点数
	CFG_POLYGON			stuDetectRegion[MAX_POLYGON_NUM];						// 检测区域
	int					nMinEnterThreshold;										// 尾随进入时间阈值最小值，低于此时间进入不报警 , 默认0 范围[0,60]，单位：秒, 默认值0
	int					nMaxEnterThreshold;										// 尾随进入时间阈值最大值，高于此时间进入不报警, 默认10 范围[0,60]，单位：秒, 默认值10
	int					nSensitivity;											// 灵敏度, 1-10, 默认值5
	int					nMaxHeight;												// 最大检测高度,单位cm
	int 				nMinHeight;												// 最小检测高度,单位cm
	BYTE                bReserved[512]; 										// 保留字节
} CFG_IVS_STEREO_TAILDETECTION_INFO;

// 事件类型EVENT_IVS_STEREO_STEREOFALLDETECTION(ATM舱内跌倒事件)对应的规则配置
typedef struct tagCFG_IVS_STEREO_STEREOFALLDETECTION_INFO
{
	...
	int					nDetectRegionPoint;										// 检测区域顶点数
	CFG_POLYGON			stuDetectRegion[MAX_POLYGON_NUM];						// 检测区域
	int					nMinDuration;											// 跌倒首次报警时间阈值，单位是秒,低于该时间不报警, 默认2 范围[0,30]
	int					nReportInterval;										// 跌倒重复报警时间,为0不重复报警, 默认60 范围[1,300]，单位：秒，默认60
	int					nSensitivity;											// 灵敏度, 1-10， 默认值5
	int					nMaxHeight;												// 最大检测高度,单位cm
	int 				nMinHeight;												// 最小检测高度,单位cm
	BYTE                bReserved[512]; 										// 保留字节
...
// 事件类型EVENT_IVS_STEREO_STAYDETECTION(ATM舱内滞留事件)对应的规则配置
typedef struct tagCFG_IVS_STEREO_STAYDETECTION_INFO
{
	...
	int					nDetectRegionPoint;										// 检测区域顶点数
	CFG_POLYGON			stuDetectRegion[MAX_POLYGON_NUM];						// 检测区域
	int					nMinDuration;											// 首次报警时间阈值，单位是秒,低于该时间不报警, 默认180, 范围[1,1200]
	int					nReportInterval;										// 跌倒重复报警时间,为0不重复报警, 默认60 范围[1,300]，单位：秒
