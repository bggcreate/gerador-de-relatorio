【Demo功能】
1、Demo演示了SDK初始化、登陆、拉流、智能交通事件上报，车辆黑白名单的增删改查，车流事件上报与历史数据查询，查询车辆图片。

【注意事项】
1、编译环境为VS2005。
2、demo目前支持的智能事件有：卡口、逆行、欠速、超速、违法停车、拥堵、行人
...
【Demo Features】
1.Demo SDK initialization,login device, real play, report intelligent traffic event, operator traffic black/white list(add, delete, modify, query), report traffic flow event and query historical data, query traffic picture.

【NOTE】
1.Complier for Demo is VS2005.
2.Demo support intelligent event are ANPR, retorgrade, lack speed, over speed, illegal parking, jam, pedestrain.
