...
void CRealPlayAndPTZControlDlg::OnBTLogin() 
{
	...
	if(bValid)
	{
		...
		LLONG lRet = CLIENT_LoginEx2(pchDVRIP, wDVRPort, pchUserName, 
		...
		if(0 != lRet)
		{
	        ...
	        m_nChannelCount = __max(deviceInfo.nChanNum, stuChn.stuVideoIn.nMaxTotal);
...
void CRealPlayAndPTZControlDlg::OnBTLeave() 
{
	...
	for(int i=0;i<9;i++)
	{
		...
		m_videoNodeInfo[i].SetVideoInfo(i+1,"",0, -1,"","",DirectMode);
		if (0 != m_lAttachVKHandle[i])
		{
			BOOL bRet = CLIENT_DetachVK(m_lAttachVKHandle[i]);
			m_lAttachVKHandle[i] = 0;
		}
	}
...
void CRealPlayAndPTZControlDlg::OnDestroy() 
{
	...
	for(int i=0;i<9;i++)
	{
		...
		if (0 != m_lAttachVKHandle[i])
		{
			CLIENT_DetachVK(m_lAttachVKHandle[i]);
			m_lAttachVKHandle[i] = 0;
		}
	}
...
void CRealPlayAndPTZControlDlg::OnButtonStop() 
{
	...
	if(CB_ERR != nIndex)
	{
		...
		//Detach vk
		if (0 != m_lAttachVKHandle[iDispNum - 1])
		{
			CLIENT_DetachVK(m_lAttachVKHandle[iDispNum - 1]);		
			m_lAttachVKHandle[iDispNum - 1] = 0;
		}
		//Refresh page 
		Invalidate();
...
static void CALLBACK AttachVK(LLONG lLoginID, LLONG lAttachHandle,int nChannelID, const NET_VKINFO* pVKInfo,LDWORD dwUser)
{
    CPlayAPI* pRealPlay = (CPlayAPI*)dwUser;
    if (pRealPlay == NULL)
    {
        return;
    }
	...
	for (int i = 0; i < 9; i++)
	{
		if (lAttachHandle == m_lAttachVKHandle[i])
		{
			nPort = i + 1;
		}
	}
    pRealPlay->PLAY_SetSecurityKey(nPort,pVKInfo->szVKID,16,pVKInfo->szVK,32);
    return;
}

void CRealPlayAndPTZControlDlg::SendSecurityKeyTo3rdPartyLib(int nChannelID, int nDispNum)
{
	NET_IN_GET_VKINFO stuIn = {sizeof(stuIn)};
	stuIn.nChannelID = nChannelID;
	NET_OUT_GET_VKINFO stuOut = {sizeof(stuOut)};
	NET_IN_ATTACH_VK stInAttach = {sizeof(stInAttach)};
	NET_OUT_ATTACH_VK stOutAttach = {sizeof(stOutAttach)};
	stInAttach.nChannelID = nChannelID;
	stInAttach.cbAttachVK = ::AttachVK;
	stInAttach.dwUser = (LDWORD)&g_PlayAPI;
	if (0 == m_lAttachVKHandle[nDispNum - 1])
	{
		m_lAttachVKHandle[nDispNum - 1] = CLIENT_AttachVK(m_LoginID,&stInAttach,&stOutAttach,5000);
	}
	if (m_lAttachVKHandle[nDispNum - 1])
	{
		BOOL bRet = CLIENT_GetVK(m_LoginID,&stuIn,&stuOut,5000);
		if (bRet)
		{
			g_PlayAPI.PLAY_SetSecurityKey(nDispNum,stuOut.stuVKInfo.szVKID,16,stuOut.stuVKInfo.szVK,32);
		}
	}
}
...
{	
	...
	if(bOpenRet)
	{
		...
		if(bPlayRet)
		{
			NET_MEDIA_ENCRYPT_INFO stuMediaEncrypt = {sizeof(stuMediaEncrypt)};
			BOOL bRet1 = CLIENT_GetConfig(m_LoginID, NET_EM_CFG_MEDIA_ENCRYPT, -1, &stuMediaEncrypt, sizeof(stuMediaEncrypt), 5000);
			if (bRet1 && stuMediaEncrypt.bKeyFrameEncryptEnable)
			{
				SendSecurityKeyTo3rdPartyLib(iChannel, iDispNum);
			}
			...
			if(0 != lRet)
			{
				...
				if (stuMediaEncrypt.bKeyFrameEncryptEnable)
				{
					CLIENT_SetEncryptRealDataCallBack(lRet, EncryptRealDataCallBackEx, (LDWORD)this);
				}
				else
				{
...
void CALLBACK EncryptRealDataCallBackEx(LLONG lRealHandle, BYTE *pBuffer, DWORD dwBufSize, LDWORD dwUser)
{
	if(dwUser == 0)
	{
		return;
	...
	dlg->ReceiveRealData(lRealHandle,0, pBuffer, dwBufSize, 0);
}
...
void CRealPlayAndPTZControlDlg::MultiPlayServerMode(int iDispNum, HWND hWnd)
{
	...
	if (bRet1 && (stuMediaEncrypt.bKeyFrameEncryptEnable))
	{
		SendSecurityKeyTo3rdPartyLib(nChannel, iDispNum);
	}
