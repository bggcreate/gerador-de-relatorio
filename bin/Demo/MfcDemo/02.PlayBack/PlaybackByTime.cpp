...
void CPlaybackByTime::OnButtonPlay(int nPlayDirection) 
{
	...
	if(bValid)
	{
		...
		if(CB_ERR != nIndex)
		{
			...
			{
				((CPlayBackDlg *)AfxGetMainWnd())->PlayBackByTimeServerMode(nChannelId, nPlayDirection);
			}
			else
			{
				((CPlayBackDlg *)AfxGetMainWnd())->PlayBackByTime(nChannelId, nPlayDirection);
			}
		}
	}
}
...
void CPlaybackByTime::OnSelectMonthcalendar(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if(bValid && (0 != m_hLoginID))
	{
		...
		//Search
		DWORD dwFindHandle = CLIENT_FindFile(m_hLoginID, nChannelId, nRecordFileType, NULL, &netTimeFrom, &netTimeTo, FALSE, 2000);
		if (dwFindHandle != 0)
		{
			int nRet = 1;
			while(nRet > 0)
