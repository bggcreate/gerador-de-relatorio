...
void CVehicleGPSDlg::OnBtnLogin()
{
	...
	LLONG lRet = CLIENT_LoginEx2(pcIP,nPort,pcUser,pcPwssd,EM_LOGIN_SPEC_CAP_TCP,NULL,&netDevInfo,&err);
	if(0 != lRet)
	{
		m_loginID = lRet;
...
{	
	...
	LLONG lRet = CLIENT_LoginEx2(szIP, nPort, m_strListenDevUser,m_strListenDevPwd,EM_LOGIN_SPEC_CAP_SERVER_CONN ,(void*)szSerial,&deviceInfo, &nError);
	if (lRet)
	{
		m_loginID = lRet;
		m_bLogined = TRUE;
...
CString CVehicleGPSDlg::ChangeToLONLAT(double dParam, int nType)
{
	...
	if (nType == 1)//经度
	{
		int nloninteger = dParam/1000000 - 180;//经纬度单位百万分之一
		double nlondecimal = (int)dParam%1000000;
		if (nloninteger > 0)
		{
			//strInfo.Format("%d.%.lf",nloninteger, nlondecimal);
			wchar_t wstrItued[32] = {0};
			swprintf(wstrItued,L"%d.%.lf°E", nloninteger, nlondecimal);
			CString strItued(wstrItued);
			strInfo = strItued;
		}
		else
		{
			//strInfo.Format("%d.%.lfW",abs(nloninteger), nlondecimal);
			wchar_t wstrItued[32] = {0};
			swprintf(wstrItued,L"%d.%.lf°W", abs(nloninteger), nlondecimal);
			CString strItued(wstrItued);
			strInfo = strItued;
		}
	}
	...
	{
		int nlatinteger = dParam/1000000 - 90;
		double nlatdecimal = (int)dParam%1000000;
		if (nlatinteger > 0)
		{
			//strInfo.Format("%d.%.lfN",nlatinteger, nlatdecimal);
			wchar_t wstrItued[32] = {0};
			swprintf(wstrItued,L"%d.%.lf°N", nlatinteger, nlatdecimal);
			CString strItued(wstrItued);
			strInfo = strItued;
		}
		else
		{
			//strInfo.Format("%d.%.lfS",abs(nlatinteger), nlatdecimal);
			wchar_t wstrItued[32] = {0};
			swprintf(wstrItued,L"%d.%.lf°S", abs(nlatinteger), nlatdecimal);
			CString strItued(wstrItued);
			strInfo = strItued;
		}
	}
