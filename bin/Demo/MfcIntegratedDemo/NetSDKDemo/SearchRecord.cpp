...
/*查询接口有两种实现方式，这里使用第一种*/
	if (!ISQUERYTYPE2) 
	{
		...
		{
			_snprintf(ifileinfo[i].filename, sizeof(ifileinfo[i].filename), "%s  channel%02d" ,pInfo->Name, m_channelsel );
		}
		OnSchClearList();
	...
	else 
	{
		...
		int i = 0;
		while (ret == 1) {
			_snprintf(ptempfileinfo->filename,sizeof(ptempfileinfo->filename),"%s  channel%02d" ,pInfo->Name, m_channelsel);
