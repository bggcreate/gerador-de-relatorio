...
void dlgCutFile::on_btnSave_clicked()
{
    ...
    DWORD newfilelen = 0;
    DWORD realFileStartPos = 0;
    DWORD realFileEndPos = 0;
    int realStartPos = 0;
    int realEndPos = 0;
	...
	DWORD nLastKeyFramePos = 0;
	DWORD nRefSize = 0;
	BYTE *pBuffer = NULL;

	BOOL bIRefValue = CPlayer::Instance()->GetIRefValue(pBuffer, &nRefSize);
	if (bIRefValue && (nRefSize > 0))
	{
		pBuffer = new BYTE[nRefSize];
		bIRefValue = CPlayer::Instance()->GetIRefValue(pBuffer, &nRefSize);
		if (bIRefValue)
		{
			FRAME_POS* pIndex = (FRAME_POS*)pBuffer;
			DWORD nIndex = nRefSize / sizeof(FRAME_POS); 
			nLastKeyFramePos = pIndex[nIndex-1].nFilePos; 
		}
		
		delete []pBuffer;
	}

	if (realFileEndPos != nLastKeyFramePos)
	{
		newfilelen = realFileEndPos - realFileStartPos;
	}
	else
	{//剪切最后一秒时，防止剪切出空数据
		newfilelen = file_in.size() - realFileStartPos;
	}
