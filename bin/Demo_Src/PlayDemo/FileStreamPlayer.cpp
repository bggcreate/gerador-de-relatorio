...
BOOL CFileStreamPlayer::GetIRefValue(PlayParam* lpPlayParam, BYTE *pBuffer, DWORD *pSize)
{
	return PLAY_GetIRefValue(lpPlayParam->nPort, pBuffer, pSize);
}
...
namespace FileStreamState
{
	...
	int CStop::Do(PlayParam* lpPlayParam)
	{
		...
		PLAY_CleanScreen(lpPlayParam->nPort,236,236,236,0,0);
		PLAY_Stop(lpPlayParam->nPort);
